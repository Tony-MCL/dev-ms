...
const en = {
  ...
  progressHero: {
    title: "Schedule planning that creates calm",
    tagline:
      "A good schedule isn’t a necessary evil. It’s a tool for clarity, predictability, and steady progress — before stress takes over.",
    lead1:
      "When tasks depend on each other, time is limited, and people rely on one another, the same need appears: to see the whole picture and understand the sequence.",
    lead2:
      "A schedule makes the workload concrete. It reveals dependencies, clarifies what must happen first — and what can run in parallel.",
    lead3:
      "Used well, it becomes a management tool: you see consequences before making decisions, and you can adjust along the way without losing control.",
    articles: {
      lead:
        "Want a bit more context before you move on? Here are two short articles explaining what a schedule is — and how to use it effectively in practice:",
  ...
  progressSell: {
    ...
    lead:
      "Once you have a plan, the key is keeping it alive: adjust when reality changes, see consequences fast, and communicate status clearly. Manage Progress is built for exactly that — without noise and without heavy setup.",
    ctaOverview: "Learn how it works",
